...
There are two ways to deploy PhoenixAI with HPA-enabled CN nodes:

### 1. Integrated PhoenixAI Cluster with CN Nodes

Deploy a complete PhoenixAI cluster including FE and CN nodes with HPA in a single chart.

### 2. Separate Cluster and Warehouse Deployment

Deploy the main PhoenixAI cluster first, then deploy separate Warehouse instances with HPA-enabled CN nodes.

## Method 1: Integrated PhoenixAI Cluster with CN Nodes

### Step 1: Create Values Configuration

Create a `phoenixai cluster-values.yaml` file:
...
### Step 2: Deploy PhoenixAI Cluster

```bash
# Add PhoenixAI Helm repository
...
$ helm install kube-anywhere phoenixai/kube-anywhere \
  -f phoenixai-cluster-values.yaml \
  --namespace phoenixai \
  --create-namespace
...
kube-anywhere-cn-autoscaler    PhoenixAICluster/kube-anywhere    15%/60%, 421%/70%   1         2         2          2m12s

$ kubectl get pods
NAME                                       READY   STATUS    RESTARTS   AGE
kube-anywhere-cn-0                         1/1     Running   0          2m17s
kube-anywhere-cn-1                         1/1     Running   0          106s
kube-anywhere-fe-0                         1/1     Running   0          3m6s
kube-anywhere-operator-5fd56d547b-7fp4m    1/1     Running   0          3m22s

# The pvc will be deleted when the CN nodes are scaled down or the cluster is deleted.
$ kubectl get pvc
NAME                          STATUS   VOLUME                                     CAPACITY   ACCESS MODES   STORAGECLASS   AGE
cn-data-kube-anywhere-cn-0    Bound    pvc-d14e41ec-eafb-44c5-8482-43a7f01e46ef   10Gi       RWO            standard       3m7s
cn-data-kube-anywhere-cn-1    Bound    pvc-9f98dc6a-194a-4284-9474-721226669d85   10Gi       RWO            standard       2m36s
cn-log-kube-anywhere-cn-0     Bound    pvc-236c7e6b-d79b-40d0-a414-d4a3b657ce2f   1Gi        RWO            standard       3m7s
cn-log-kube-anywhere-cn-1     Bound    pvc-cd8f2d22-1368-4830-93a0-f19f9d67ede9   1Gi        RWO            standard       2m36s
fe-log-kube-anywhere-fe-0     Bound    pvc-9ef1467b-992b-4693-af92-c18214f91188   1Gi        RWO            standard       3m56s
fe-meta-kube-anywhere-fe-0    Bound    pvc-522e40b4-8a48-4a62-a593-1ca9174a1475   10Gi       RWO            standard       3m56s
```
...
### Step 1: Deploy Main PhoenixAI Cluster
...
$ kubectl get pods
NAME                                       READY   STATUS    RESTARTS   AGE
kube-anywhere-fe-0                         1/1     Running   0          73s
kube-anywhere-operator-5fd56d547b-46c9h    1/1     Running   0          91s

# The FE PVCS will be deleted when the PhoenixAI cluster is deleted.
$ kubectl get pvc
NAME                          STATUS   VOLUME                                     CAPACITY   ACCESS MODES   STORAGECLASS   AGE
fe-log-kube-anywhere-fe-0     Bound    pvc-5fce212f-0f22-494c-925d-ffd17bada556   1Gi        RWO            standard       75s
fe-meta-kube-anywhere-fe-0    Bound    pvc-74b5ea92-c298-48eb-a7bf-5682679c4d7e   10Gi       RWO            standard       75s  
```
...
```yaml
spec:
  # Reference to the main PhoenixAI cluster
...
```bash
$ helm install wh-1 phoenixai/warehouse \
  -f warehouse-values.yaml \
  --namespace phoenixai

$ kubectl get pods
NAME                                       READY   STATUS    RESTARTS   AGE
kube-anywhere-fe-0                         1/1     Running   0          4m15s
kube-anywhere-operator-5fd56d547b-46c9h    1/1     Running   0          4m33s
wh-1-warehouse-cn-0                        1/1     Running   0          20s

$ kubectl get hpa
NAME                           REFERENCE                 TARGETS                        MINPODS   MAXPODS   REPLICAS   AGE
wh-1-warehouse-cn-autoscaler   PhoenixAIWarehouse/wh-1   <unknown>/60%, <unknown>/70%   1         2         1          22s  
...
fe-log-kube-anywhere-fe-0     Bound    pvc-5fce212f-0f22-494c-925d-ffd17bada556   1Gi        RWO            standard       5m50s
fe-meta-kube-anywhere-fe-0    Bound    pvc-74b5ea92-c298-48eb-a7bf-5682679c4d7e   10Gi       RWO            standard       5m50s
```
...
$ kubectl get pods
NAME                                       READY   STATUS    RESTARTS   AGE
kube-anywhere-fe-0                         1/1     Running   0          13m
kube-anywhere-operator-5fd56d547b-46c9h    1/1     Running   0          13m
...
fe-log-kube-anywhere-fe-0     Bound    pvc-5fce212f-0f22-494c-925d-ffd17bada556   1Gi        RWO            standard       13m
fe-meta-kube-anywhere-fe-0    Bound    pvc-74b5ea92-c298-48eb-a7bf-5682679c4d7e   10Gi       RWO            standard       13m
