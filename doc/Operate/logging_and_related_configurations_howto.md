# PhoenixAI Logging and Related Configurations

PhoenixAI is composed of two components: FE and CN. Among them, FE is essential. This document describes the
logging and associated configurations for PhoenixAI components, including:
...
2. The CN component's logs are located in: `/opt/starrocks/cn/log`, with key logs
...
> Attention: The Operator will create PVC resources for the PhoenixAI cluster. The storage class controller will then
> automatically generate the specific storage volume.

### 3.1 Helm Chart Supports Persisting Logs

If you deployed the PhoenixAI cluster using Helm Chart, you can modify the `values.yaml` content to persist logs. Here's
an example for the FE component:

For the kube-anywhere Helm Chart, you can configure as:
...
For the phoenixai Helm Chart, configure as:
...
If you've deployed the PhoenixAI cluster using Helm Chart, you can modify the `values.yaml` content to set environment
variables. An example for the FE component:

For the kube-anywhere Helm Chart, configure as:
...
Refer to: [Datadog](../Monitor/integration-with-datadog.md).
