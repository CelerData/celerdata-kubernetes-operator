...
This document describes the `--fe-ssl-mode` flag (Helm value `celerDataOperator.feSslMode`) that lets
...
Set `celerDataOperator.feSslMode` when installing or upgrading the operator (or the `kube-celerdata`
umbrella chart, where the same key lives under `operator.celerDataOperator.feSslMode`):
